...
export default function CNPage() {
    const labs = getLabsBySubject("CN");
    ...
    useEffect(() => {
        ...
        setOverallCompletion(getSubjectCompletionRate("cn", labIds));
        ...
        <div className="min-h-screen bg-white text-black font-sans selection:bg-black selection:text-white pb-10">
            <header className="bg-white border-b-2 border-black/5 sticky top-0 z-20 backdrop-blur-md bg-white/80">
                <div className="container mx-auto px-4 py-4 flex items-center gap-4">
                    ...
                    <h1 className="text-lg font-bold uppercase tracking-tight">Computer Networks</h1>
                </div>
            </header>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                        <div className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <Progress value={overallCompletion} className="h-4 bg-black/5 rounded-full" indicatorClassName={isFullyCompleted ? "bg-blue-600" : "bg-black"} />
                            </div>
                        </div>
                        ...
                        <div className={`p-8 rounded-2xl border-2 flex flex-col items-center justify-center text-center gap-4 transition-all ${isFullyCompleted
                            ...
                            {isFullyCompleted ? (
                                <>
                                    ...
                                    <div>
                                        <h3 className="font-black text-blue-900 text-xl uppercase tracking-tighter">Subject Completed!</h3>
                                        <p className="text-sm text-blue-700 mt-1 font-medium">Download your official certification of excellence.</p>
                                    </div>
                                    <Button
                                        onClick={handleGenerateFinalCertificate}
                                        className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black h-14 rounded-xl shadow-lg hover:shadow-xl transition-all uppercase tracking-widest border-b-4 border-blue-800 active:border-b-0 active:translate-y-1"
                                    >
                        ...
                        return (
                            <div key={p.id} className="group flex flex-col md:flex-row md:items-center justify-between p-6 border-b-2 border-black/5 hover:bg-gray-50 transition-colors gap-6">
                                <div className="flex items-start gap-6 flex-1">
                                    <div className={`hidden md:flex flex-col items-center justify-center w-16 h-16 border-2 ${isCompleted ? 'border-blue-600 bg-blue-600 text-white' : 'border-black/10 text-black'} rounded-lg transition-all`}>
                                    ...
                                    <div className="space-y-3 w-full max-w-xl">
                                        <div>
                                            ...
                                            <h3 className="text-xl font-bold uppercase tracking-tight group-hover:text-blue-600 transition-colors leading-none">{p.metadata.title}</h3>
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <Progress value={completion} className="h-2 bg-black/5" indicatorClassName={isCompleted ? "bg-blue-600" : "bg-black"} />
                                        </div>
                                    </div>
                                </div>

                                <div className="flex items-center gap-3 shrink-0">
                                    <Link href={`/dashboard/cn/${p.id}`} className="w-full md:w-auto">
