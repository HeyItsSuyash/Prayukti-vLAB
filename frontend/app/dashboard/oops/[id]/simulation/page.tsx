...
export default function SimulationPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    const labId = id.startsWith('oops-exp-') ? id : `oops-exp-${id}`; // Adjust prefix if needed
        ...
        <div className="flex flex-col h-screen bg-white">
            <header className="bg-white border-b-2 border-black/5 px-4 py-3 flex items-center justify-between shrink-0 h-16">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="font-bold uppercase tracking-tight text-black">OOPS Workbench <span className="text-gray-400 text-sm font-medium ml-2 hidden sm:inline-block">| {id}</span></h1>
                </div>
            </header>
