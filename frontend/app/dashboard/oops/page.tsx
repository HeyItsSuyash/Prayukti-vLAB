...
import JavaCompiler from "@/components/oops/JavaCompiler";
import { useState } from "react";
import { Code2, BookOpen } from "lucide-react";

export default function OOPSPage() {
    const labs = getLabsBySubject("OOPS");
    const [view, setView] = useState<"experiments" | "compiler">("experiments");
        ...
        <div className="min-h-screen bg-white text-black font-sans selection:bg-black selection:text-white">
            <header className="bg-white border-b-2 border-black/5 sticky top-0 z-20 backdrop-blur-md bg-white/80">
                <div className="container mx-auto px-4 py-4 flex items-center justify-between">
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-lg border-2 border-black/5">
                        <button
                            onClick={() => setView("experiments")}
                            className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-xs font-bold uppercase tracking-wider transition-all ${view === "experiments" ? "bg-black text-white shadow-lg" : "text-gray-500 hover:text-black"}`}
                        >
                            <BookOpen className="h-3.5 w-3.5" /> Experiments
                        </button>
                        <button
                            onClick={() => setView("compiler")}
                            className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-xs font-bold uppercase tracking-wider transition-all ${view === "compiler" ? "bg-black text-white shadow-lg" : "text-gray-500 hover:text-black"}`}
                        >
                            <Code2 className="h-3.5 w-3.5" /> Compiler
                        </button>
                    </div>
                </div>
            </header>
            ...
            <main className="container mx-auto px-4 py-12">
                {view === "experiments" ? (
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="mb-12">
                            <h2 className="text-4xl lg:text-5xl font-black uppercase tracking-tighter mb-2">Java Playground</h2>
                            <p className="text-lg text-gray-500 font-medium">Write, compile, and execute Java OOPS code in real-time.</p>
                        </div>
                        <JavaCompiler />
                    </div>
                )}
