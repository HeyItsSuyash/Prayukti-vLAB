...
export default function OOPSPage() {
    ...
    useEffect(() => {
        ...
        const subjectProgress = getSubjectProgress("oops");
        const labIds = labs.map(p => p.id);

        setStudentNameStr(studentName);
        setProgress(subjectProgress);
        setOverallCompletion(getSubjectCompletionRate("oops", labIds));
        setIsFullyCompleted(isSubjectCompleted("oops", labIds));
    }, [labs]);
    ...
    const handleGenerateFinalCertificate = async () => {
        ...
        await generateCertificate(studentNameStr, "Object Oriented Programming", true);
    };
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                        <div className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <Progress value={overallCompletion} className="h-4 bg-black/5 rounded-full" indicatorClassName={isFullyCompleted ? "bg-purple-600" : "bg-black"} />
                            </div>
                        </div>
                        ...
                        <div className={`p-8 rounded-2xl border-2 flex flex-col items-center justify-center text-center gap-4 transition-all ${isFullyCompleted
                            ...
                            {isFullyCompleted ? (
                                <>
                                    <div className="w-16 h-16 bg-purple-600 rounded-full flex items-center justify-center shadow-lg shadow-purple-200">
                                        <Award className="h-10 w-10 text-white" />
                                    </div>
                                    <div>
                                        <h3 className="font-black text-purple-900 text-xl uppercase tracking-tighter">Subject Completed!</h3>
                                        <p className="text-sm text-purple-700 mt-1 font-medium">Download your official certification of excellence.</p>
                                    </div>
                                    <Button
                                        onClick={handleGenerateFinalCertificate}
                                        className="w-full bg-purple-600 hover:bg-purple-700 text-white font-black h-14 rounded-xl shadow-lg hover:shadow-xl transition-all uppercase tracking-widest border-b-4 border-purple-800 active:border-b-0 active:translate-y-1"
                                    >
                        ...
                        return (
                            <div key={p.id} className="group flex flex-col md:flex-row md:items-center justify-between p-6 border-b-2 border-black/5 hover:bg-gray-50 transition-colors gap-6">
                                <div className="flex items-start gap-6 flex-1">
                                    <div className={`hidden md:flex flex-col items-center justify-center w-16 h-16 border-2 ${isCompleted ? 'border-purple-600 bg-purple-600 text-white' : 'border-black/10 text-black'} rounded-lg transition-all`}>
                                    ...
                                    <div className="space-y-3 w-full max-w-xl">
                                        <div>
                                            ...
                                            <h3 className="text-xl font-bold uppercase tracking-tight group-hover:text-purple-600 transition-colors leading-none">{p.metadata.title}</h3>
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <Progress value={completion} className="h-2 bg-black/5" indicatorClassName={isCompleted ? "bg-purple-600" : "bg-black"} />
                                        </div>
                                    </div>
                                </div>

                                <div className="flex items-center gap-3 shrink-0">
                                    <Link href={`/dashboard/oops/${p.id}`} className="w-full md:w-auto">
