                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                        <div className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <Progress value={overallCompletion} className="h-4 bg-black/5 rounded-full" indicatorClassName={isFullyCompleted ? "bg-green-600" : "bg-black"} />
                            </div>
                        </div>
                        ...
                        <div className={`p-8 rounded-2xl border-2 flex flex-col items-center justify-center text-center gap-4 transition-all ${isFullyCompleted
                            ...
                            {isFullyCompleted ? (
                                <>
                                    <div className="w-16 h-16 bg-green-600 rounded-full flex items-center justify-center shadow-lg shadow-green-200">
                                        <Award className="h-10 w-10 text-white" />
                                    </div>
                                    <div>
                                        <h3 className="font-black text-green-900 text-xl uppercase tracking-tighter">Subject Completed!</h3>
                                        <p className="text-sm text-green-700 mt-1 font-medium">Download your official certification of excellence.</p>
                                    </div>
                                    <Button
                                        onClick={handleGenerateFinalCertificate}
                                        className="w-full bg-green-600 hover:bg-green-700 text-white font-black h-14 rounded-xl shadow-lg hover:shadow-xl transition-all uppercase tracking-widest border-b-4 border-green-800 active:border-b-0 active:translate-y-1"
                                    >
                        ...
                        return (
                            <div key={p.id} className="group flex flex-col md:flex-row md:items-center justify-between p-6 border-b-2 border-black/5 hover:bg-gray-50 transition-colors gap-6">
                                <div className="flex items-start gap-6 flex-1">
                                    <div className={`hidden md:flex flex-col items-center justify-center w-16 h-16 border-2 ${isCompleted ? 'border-green-600 bg-green-600 text-white' : 'border-black/10 text-black'} rounded-lg transition-all`}>
                                    ...
                                    <div className="space-y-3 w-full max-w-xl">
                                        <div>
                                            ...
                                            <h3 className="text-xl font-bold uppercase tracking-tight group-hover:text-green-600 transition-colors leading-none">{p.metadata.title}</h3>
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <Progress value={completion} className="h-2 bg-black/5" indicatorClassName={isCompleted ? "bg-green-600" : "bg-black"} />
                                        </div>
                                    </div>
                                </div>
